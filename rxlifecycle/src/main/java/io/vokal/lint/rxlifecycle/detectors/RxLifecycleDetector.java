...
//        log(nodeInfo(previous)); // keep for debugging
    ...
    static String nodeInfo(AbstractInsnNode node) {
        Class<? extends AbstractInsnNode> clz = node.getClass();
        String owner = "?";
        try {
            owner = (String) clz.getField("owner").get(node);
        } catch (NoSuchFieldException | IllegalAccessException e) {
            // ignore
        }
        String name = "?";
        try {
            name = (String) clz.getField("name").get(node);
        } catch (NoSuchFieldException | IllegalAccessException e) {
            // ignore
        }
        return String.format("%s: %s .%s", clz.getSimpleName(), owner, name);
    }
